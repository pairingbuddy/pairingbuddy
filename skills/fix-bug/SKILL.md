---
name: fix-bug
description: Fix bugs using TDD with reproduction tests and full coverage
---

# Fix Bug

Fix existing bugs using TDD.

## Workflow Rules (CRITICAL - Follow Exactly)

**Phase 0: REPRODUCE** (Mandatory)
- Read [phase-0-reproduce](modules/phase-0-reproduce.md)
- Write reproduction test that demonstrates the bug
- Enumerate any additional test scenarios needed

**Phase RED-GREEN Cycle** (Mandatory - No Skipping)

For EVERY test you MUST execute:
1. **RED** → Read [phase-red](_shared/modules/phase-red.md) - Write ONE failing test (or verify reproduction test fails)
2. **GREEN** → Read [phase-green](_shared/modules/phase-green.md) - Fix the bug to make test pass with minimal code
